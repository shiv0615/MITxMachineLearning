...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if p < epsilon:
        ...
        indices = np.argmax((theta @ state_vector))
        action_index, object_index = index2tuple(indices)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    # TODO Your code here
    q_max = 0
    if not terminal: q_max = np.max(theta @ next_state_vector)
    ...
    theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + \
                                                     ALPHA * (reward + GAMMA * q_max - q_val) * current_state_vector
# pragma: coderesponse end
