...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if p < epsilon:
        ...
        action_index, object_index = np.unravel_index(np.argmax(q_func[state_1, state_2, :, :], axis=None),
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    """
    q_max = 0
    if not terminal: q_max = np.max(q_func[next_state_1, next_state_2, :, :])
    q_func[current_state_1, current_state_2, action_index, object_index] = (1 - ALPHA) * q_func[
        current_state_1, current_state_2, action_index, object_index] + ALPHA * (reward + GAMMA * q_max)
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        if not for_training:
            # update reward
            epi_reward = epi_reward + np.power(GAMMA,icount)*reward
