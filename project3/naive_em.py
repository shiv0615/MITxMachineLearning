...
def estep(X: np.ndarray, mixture: GaussianMixture) -> Tuple[np.ndarray, float]:
    ...
    cost = 0
    for j in range(K):
        post[:, j] = np.exp(-(0.5 / mixture.var[j])*np.square(np.linalg.norm((X - mixture.mu[j]), axis=1)))
        post[:, j] = np.multiply(fac[j]*post[:, j],mixture.p[j])
        gauss_sum[:] += post[:, j]
    for j in range(K):
        post[:, j] = post[:, j]/gauss_sum[:]
        cost += np.sum(post[:,j] * np.log(gauss_sum[:]))
    return (post, cost)
